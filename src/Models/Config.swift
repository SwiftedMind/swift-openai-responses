...
public struct TextConfig: Equatable, Hashable, Codable, Sendable {
  ...
  @Codable @CodedAt("type") @CodingKeys(.snake_case) public enum Format: Equatable, Hashable, Sendable {
    ...
    public static func == (lhs: TextConfig.Format, rhs: TextConfig.Format) -> Bool {
      switch (lhs, rhs) {
      case (.jsonObject, .jsonObject):
        return true
      case let (.jsonSchema(lschema, ldescription, lname, lstrict), .jsonSchema(rschema, rdescription, rname, rstrict)):
        return lschema == rschema && ldescription == rdescription && lname == rname && lstrict == rstrict
      case let(.generationSchema(_, ldescription, lname, lstrict), .jsonSchema(_, rdescription, rname, rstrict)):
        // TODO: Add some kind of support for GenerationSchema equality
        return ldescription == rdescription && lname == rname && lstrict == rstrict
      default:
        return false
      }
    }
